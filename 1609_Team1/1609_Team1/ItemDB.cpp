...
void ItemDB::Init()
{
	...
	// 상점 구매 아이템(소모품)
	Add(new ItemData(ITEM_USINGITEM, 1005, 100, 0, 5.0f,  0,   0, 0, 50,   TEXT("수류탄(Grenade)"),
		TEXT("가격이 저렴한 수류탄. \n손에 힘이 없어서 멀리는 던지지 못한다.")));
	Add(new ItemData(ITEM_USINGITEM, 1006, 200, 0, 15.0f, 0,   0, 0, 3000, TEXT("공중 폭격(Air Bomb"),
		TEXT("광범위하게 살상하는 공중폭격. \n무전기로 본부를 호출해서 지원을 요청한다.")));
	Add(new ItemData(ITEM_USINGITEM, 1007,  30, 0, 10.0f, 0,   0, 0, 1000, TEXT("용암지대(Rava Region)"),
		TEXT("지속적인 화상상태를 입히는 용암지대. \n용암이 담긴 캡슐을 던져 용암지대를 형성한다.")));
	Add(new ItemData(ITEM_USINGITEM, 1008,   0, 0, 0.0f,  100, 0, 0, 200,  TEXT("벙커 수리(Bunker Repair)"),
		TEXT("금이 간 벙커를 수리. 몬스터로부터 공격당해 금이 간 벙커를 수리한다.")));

	// 상점 구매 아이템(총알)
	Add(new ItemData(ITEM_BULLET, 1009, 0, 0, 0, 0, 0, 10,  50,  TEXT("권총 탄약(Pistol Bullet)"),
		TEXT("권총의 탄알집")));
